...
// Captures console.log/warn/error and sends to Desafio Minerva Foods server for AI debugging
...
const getLogServerUrl = (): string | null => {
  ...
  try {
    if (Platform.OS === 'web' && typeof window !== 'undefined') {
      ...
      cachedLogServerUrl = `${window.location.origin}/desafio-minerva-foods-logs`;
    } else {
      ...
      if (experienceUrl) {
        ...
        cachedLogServerUrl = `${baseUrl}/desafio-minerva-foods-logs`;
      } else {
        // Fallback: try to use manifest hostUri
        const hostUri = Constants.expoConfig?.hostUri || (Constants as any).manifest?.hostUri;
        if (hostUri) {
          const protocol = hostUri.includes('ngrok') || hostUri.includes('.io') ? 'https' : 'http';
          cachedLogServerUrl = `${protocol}://${hostUri.split('/')[0]}/desafio-minerva-foods-logs`;
        }
      }
    }
  } catch (e) {
      ...
      fetch(url, {
        ...
        if (!fetchErrorLogged) {
          ...
          if (typeof window !== 'undefined' && window.console) {
            (window.console as any).__proto__.log.call(console, '[Desafio Minerva Foods] Fetch error (will not repeat):', e.message || e);
          }
        }
      });
    } catch (e) {
...
export const setupErrorLogging = () => {
  ...
  originalConsoleLog('[Desafio Minerva Foods] Setting up error logging...');
  originalConsoleLog('[Desafio Minerva Foods] Log server URL:', logServerUrl || 'NOT AVAILABLE');
  originalConsoleLog('[Desafio Minerva Foods] Platform:', Platform.OS);
