...
const LOG_FILE_PATH = path.join(__dirname, '.desafio-minerva-foodss', 'app_console.log');
...
config.server.enhanceMiddleware = (middleware) => {
  return (req, res, next) => {
    ...
    if (pathname === '/desafio-minerva-foodss-logs' && req.method === 'POST') {
      console.log('[Desafio Minerva Foods-LOGS] Received POST request');
      let body = '';
      req.on('data', chunk => {
        body += chunk.toString();
      });
      req.on('end', () => {
        try {
          ...
          console.log('[Desafio Minerva Foods-LOGS] Writing log:', logLine.trim());
          ...
          console.error('[Desafio Minerva Foods-LOGS] Error processing log:', e.message);
          res.writeHead(500, {
            'Content-Type': 'application/json',
            'Access-Control-Allow-Origin': '*',
          });
    ...
    if (pathname === '/desafio-minerva-foodss-logs' && req.method === 'OPTIONS') {
      console.log('[Desafio Minerva Foods-LOGS] Received OPTIONS preflight request');
      res.writeHead(200, {
        'Access-Control-Allow-Origin': '*',
